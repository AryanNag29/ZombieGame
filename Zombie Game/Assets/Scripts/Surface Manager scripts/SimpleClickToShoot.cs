...
namespace ZombieGame
{
    ...
    public class SimpleClickToShoot : MonoBehaviour
    {
        ...
        [SerializeField] private PoolableObject BulletTrail;
        ...
        public void Shoot()
        {
            if (LastShootTime + ShootDelay < Time.time)
            {
                ...
                {
                    StartCoroutine(SpawnTrail(trail, hit.collider, hit.point, hit.normal, hit.triangleIndex,
                        BounceDistance, true));
                }
                else
                {
        ...
        private IEnumerator SpawnTrail(TrailRenderer Trail, Collider HitCollider, Vector3 HitPoint, Vector3 HitNormal,
            int TriangleIndex, float BounceDistance, bool MadeImpact)
        {
            ...
            if (MadeImpact)
            {
                SurfaceManager.Instance.HandleImpact(HitCollider.gameObject, HitPoint, HitNormal, ImpactType,
                    TriangleIndex);

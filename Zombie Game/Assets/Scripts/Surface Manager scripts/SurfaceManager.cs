...
namespace ZombieGame
{
    public class SurfaceManager : MonoBehaviour
    {
        ...
        private Texture GetActiveTextureFromRenderer(Renderer Renderer, int TriangleIndex)
        {
            ...
            Debug.LogError(
                $"{Renderer.name} has no MeshFilter! Using default impact effect instead of texture-specific one because we'll be unable to find the correct texture!");
            return null;
        }
        ...
        private void PlayEffects(Vector3 HitPoint, Vector3 HitNormal, SurfaceEffect SurfaceEffect, float SoundOffset)
        {
            foreach (SpawnObjectEffect spawnObjectEffect in SurfaceEffect.SpawnObjectEffects)
            {
                ...
                {
                    ObjectPool pool = ObjectPool.CreateInstance(spawnObjectEffect.Prefab.GetComponent<PoolableObject>(),
                        DefaultPoolSizes);

                    PoolableObject instance =
                        pool.GetObject(HitPoint + HitNormal * 0.001f, Quaternion.LookRotation(HitNormal));
            ...
            foreach (PlayAudioEffect playAudioEffect in SurfaceEffect.PlayAudioEffects)
            {
                ...
                ObjectPool pool =
                    ObjectPool.CreateInstance(playAudioEffect.AudioSourcePrefab.GetComponent<PoolableObject>(),
                        DefaultPoolSizes);
